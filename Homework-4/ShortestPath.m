function [shortestPaths, totalCosts] = ShortestPath(map, s, d, k_paths)

if s > size(map,1) || d > size(map,1)
    warning('The s or d node are not part of map');
    shortestPaths=[];
    totalCosts=[];
else
    
    k=1;
    [path cost] = dijkstra(map, s, d);
        if isempty(path)
        shortestPaths=[];
        totalCosts=[];
    else
        ...
        while (k < k_paths   &&   size_X ~= 0 )
            ...
            for index_dev_vertex= w_index_in_path: length(P_) - 1  
                ...
                for j = 1: length(SP_sameSubPath)
                    ...
                    temp_map(v_,next)=inf;   
                end
                
                sub_P = P_(1:index_dev_vertex);
                cost_sub_P=0;
                for i = 1: length(sub_P)-1
                    cost_sub_P = cost_sub_P + map(sub_P(i),sub_P(i+1));
                end
                  
                [dev_p c] = dijkstra(temp_map, P_(index_dev_vertex), d);
                if ~isempty(dev_p)
